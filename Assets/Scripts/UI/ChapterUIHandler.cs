...
using EchoOfTheTimes.Persistence; // Пространство имен для PersistenceService
using EchoOfTheTimes.SceneManagement; // Пространство имен для GameChapter и StatusType
...
public class ChapterUIHandler : MonoBehaviour
{
    ...
    private void SetChapterColor(Color color)
    {
        if (_chapterImage != null)
        {
            _chapterImage.color = color;
        }
        else
        {
