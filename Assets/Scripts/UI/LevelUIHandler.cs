...
using EchoOfTheTimes.Persistence; // Пространство имен для сохранений
using EchoOfTheTimes.SceneManagement; // Пространство имен для работы с уровнями
...
public class LevelUIHandler : MonoBehaviour
{
    ...
    private void SetLevelColor(Color color)
    {
        if (_levelImage != null)
        {
            _levelImage.color = color;
        }
        else
        {
            Debug.LogError("Компонент Image не найден на уровне.");
        }
    }
}
